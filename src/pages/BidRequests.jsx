...
const BidRequests = () => {
  ...
  console.log(isLoading);
    ...
    const getData=async()=>{
      ...
      // const {data}=await axios(`${import.meta.env.VITE_API_URL}/my-requests/${user?.email}`)
...
//   handleStatus
  const handleStatus=async(id,prevStatus,status)=>{
    if(prevStatus===status) return toast.error(`its already in ${status}`)
    // const {data}=await axios.patch(`${import.meta.env.VITE_API_URL}/bid/${id}`,{status})
  ...
  if(isLoading) return <div>data is still Loading...</div>
    return (
